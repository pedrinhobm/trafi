...
% Parámetros del sistema ON/OFF para cada escenario
a_values = [0.01, 0.03, 0.1];
b_values = [0.02, 0.06, 0.2];
...
% Graficar y analizar el escenario A
figure;
subplot(3, 2, 1);
...
for i = 1:length(a_values)
    ...
    % Subgráfica para cada caso del escenario A
    subplot(3, 2, i);
    hold on;
    ...
    title(['Escenario A - Caso ' num2str(i)]);
    xlabel('Tiempo (s)');
    ylabel('Probabilidad');
...
annotation('textbox', [0 0.9 1 0.1], 'String', 'Evolución de la PMF vs Tiempo - Escenario A', 'EdgeColor', 'none', 'HorizontalAlignment', 'center', 'FontSize', 14);
